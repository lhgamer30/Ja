...
local HasAlreadyEnteredMarker, isDead, cuffed, hasAlreadyJoined, playerInService = false, false, false, false, false
...
function OpenArmoryMenu(station)
	...
	if Config.EnableArmoryManagement then
		...
		table.insert(elements, {label = _U('get_proof'),      value = 'get_proof'})
		table.insert(elements, {label = _U('get_clip'),      value = 'get_clip'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory', {
		...
		elseif data.current.value == 'put_weapon' then
			OpenPutWeaponMenu()
		elseif data.current.value == 'get_proof' then
			AddArmourToPed(GetPlayerPed(-1), 100)
		elseif data.current.value == 'get_clip' then
				ped = GetPlayerPed(-1)
				if IsPedArmed(ped, 4) then
				  ...
				  if hash~=nil then
					AddAmmoToPed(GetPlayerPed(-1), hash,1000)
					  ...
					  TriggerEvent('notifications', "#14e309", "NAME", "Du hast keine Waffe in der Hand")
				  end
				else
				  TriggerEvent('notifications', "#14e309", "NAME", "Du hast keine Waffe in der Hand")
				end
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		css	 = 'superete',
		title    = 'Polizeikommissariat 15',
		align    = 'top-left',
		elements = {
			{label = _U('citizen_interaction'), value = 'citizen_interaction'},
			{label = _U('vehicle_interaction'), value = 'vehicle_interaction'},
			{label = _U('rd_mobile'), value = 'rd_mobile'},
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				...
				{label = _U('sendToJail'), value = 'sendToJail'}
			}
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'fine' then
						OpenRoksFineMenu(closestPlayer)
					elseif action == 'license' then
						ShowPlayerLicense(closestPlayer)
					elseif action == 'unpaid_bills' then
						OpenUnpaidBillsMenu(closestPlayer)
					elseif data.current.value == "sendToJail" then
						local playerId = GetPlayerServerId(PlayerId()) -- ID des ausgewählten Spielers
						local time = 300 -- Zeit in Sekunden (Beispiel: 300 Sekunden = 5 Minuten)
			
						TriggerServerEvent('Jail:policeSendToJail', playerId, time)
						ESX.UI.Menu.CloseAll()
					end
				else
...
function OpenRoksFineMenu(player)

	ESX.UI.Menu.Open(
		'dialog', GetCurrentResourceName(), 'fine',
		{
		  title = "Grund"
		},
	function(data, menu)
		...
		ESX.UI.Menu.Open(
			...
			function(data2, menu2)
				...
				if price == nil then
					...
					TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_police', reason, price)
				end
...
function OpenVehicleInfosMenu(vehicleData)
    ESX.TriggerServerCallback('esx_policejob:getVehicleInfos', function(retrivedInfo)
        ...
        if not retrivedInfo.owner then
            ...
            if retrivedInfo.registrated == 'yes' then
                table.insert(elements, {label = 'Versicherung: Angemeldet'})
            else
                table.insert(elements, {label = 'Versicherung: Abgemeldet'})
            end
        end
...
-- Hier ein Beispiel, wie du die Option zum Spieler ins Gefängnis schicken hinzufügen könntest
RegisterNetEvent('esx_policejob:openMenu')
AddEventHandler('esx_policejob:openMenu', function()
    local elements = {
        { label = "Spieler ins Gefängnis schicken", value = "sendToJail" }
        -- Füge hier weitere Menüoptionen für die Polizei hinzu
    }
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
        ...
        if data.current.value == "sendToJail" then
            local playerId = GetPlayerServerId(PlayerId()) -- ID des ausgewählten Spielers
            local time = 300 -- Zeit in Sekunden (Beispiel: 300 Sekunden = 5 Minuten)
...
Citizen.CreateThread(function()
	while true do
		...
		if cuffed then
			...
			DisableControlAction(0, 23, true) -- Get out/in Vehicle
